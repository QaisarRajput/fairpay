...
function summarizeCoverage(seriesList) {
    const starts = seriesList.map((series) => series.points[0]?.date).filter(Boolean);
...
async function run() {
    ...
    if (malformedRows > MAX_SKIP_COUNT || malformedRatio > MAX_SKIP_RATIO) {
        throw new Error(`ETL drift threshold exceeded: malformed ${malformedRows}/${totalRows} rows (${(malformedRatio * 100).toFixed(2)}%).`);
    }
    ...
    console.log(`ETL completed: ${seriesList.length} series, ${totalRows} rows, ${malformedRows} malformed, ${missingValueRows} missing-value markers, ${changedFiles.length} file(s) changed.`);
}
run().catch((error) => {
